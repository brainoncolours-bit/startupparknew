...
function Scene({ scrollProgress }) {
  ...
  useFrame(() => {
    ...
    modelRef.current.rotation.x = Math.sin(currentScroll * Math.PI) * 0.15 + 0.05;
    
    // Dynamic X-axis shift: keeps it centered early on, shifts slightly left later
    modelRef.current.position.x = currentScroll < 0.3 ? 0 : (currentScroll - 0.3) * -0.8;
  });
...
export default function CardSection({ onPreBook }) {
  ...
  const displayStep2 = useTransform(smoothScroll, [0.33, 0.85], ["grid", "none"]);
        ...
        <div className="lg:col-span-7 h-full relative z-20 flex items-center selection:bg-blue-500/30">
          ...
          >
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-neutral-900 border border-neutral-800 text-neutral-400 text-xs font-mono tracking-wider w-fit">
              <Sparkles className="w-3.5 h-3.5 text-blue-400" /> xLimited Edition
            </div>
            <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-neutral-100 leading-tight">
              The Next Gen <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-neutral-100 to-neutral-400">Founder Card</span>
            </h1>
            <p className="text-neutral-400 text-base leading-relaxed font-normal">
              It unlocks the Startup Park ecosystem. With this card, a founder gets access to premium utilities designed to scale ideas into empires.
            </p>
          </motion.div>
          ...
          <motion.div 
            style={{ 
              opacity: useTransform(smoothScroll, [0.35, 0.42, 0.78, 0.85], [0, 1, 1, 0]),
          ...
          >
            {benefits.map((benefit, idx) => (
              <div key={idx} className="flex items-start gap-3 border border-neutral-900 bg-neutral-950/40 p-3 rounded-lg backdrop-blur-sm">
                <CheckCircle2 className="w-4 h-4 text-blue-500 shrink-0 mt-0.5" />
                <span className="text-xs md:text-sm text-neutral-300 font-medium leading-normal">{benefit}</span>
              </div>
            ))}
          </motion.div>
          ...
          >
            <div className="space-y-2">
              <h2 className="text-2xl md:text-3xl font-bold tracking-tight">Secure Your Founder Allocation</h2>
              <p className="text-xs text-neutral-500 leading-relaxed font-mono">
                Deploy infrastructure adjustments, prioritize ecosystem node mapping, and unlock priority processing queues instantly.
              </p>
            </div>
