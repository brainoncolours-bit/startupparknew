import React, {
  useLayoutEffect, useRef, Suspense, useEffect,
...
/** Fade-in + slide-up a single ref on scroll enter */
function useFadeIn(ref, { y = 30, duration = 0.9, ease = "power3.out", delay = 0, start = "top 75%", extraFrom = {}, extraTo = {} } = {}) {
  useLayoutEffect(() => {
    if (!ref.current) return;
    const ctx = gsap.context(() => {
      gsap.fromTo(ref.current,
        { y, autoAlpha: 0, ...extraFrom },
        { y: 0, autoAlpha: 1, duration, ease, delay, ...extraTo,
...
const TIERS = [
  {
    id: "founder", label: "Founder", tagline: "Begin your journey",
    price: "₹4,999", period: "/ year", badge: null,
    borderColor: "rgba(255,255,255,0.12)", glowColor: "rgba(255,255,255,0.06)",
    featured: false, cta: "Get Started",
    perks: [
      "Hot-desk access (10 days/month)", "Community Slack & events",
      "Monthly masterclass access", "Startup directory listing",
      "Pitch deck review (1x/quarter)",
    ],
  },
  {
    id: "accelerator", label: "Accelerator", tagline: "Scale with velocity",
    price: "₹14,999", period: "/ year", badge: "Most Popular",
    borderColor: "rgba(255,255,255,0.38)", glowColor: "rgba(255,255,255,0.10)",
    featured: true, cta: "Join Now",
    perks: [
      "Dedicated desk (unlimited access)", "Priority investor intros",
      "Weekly 1:1 mentor sessions", "Legal & cap-table support",
      "Demo Day participation", "Full event suite access",
      "Funding milestone tracking",
    ],
  },
  {
    id: "visionary", label: "Visionary", tagline: "Lead the ecosystem",
    price: "₹39,999", period: "/ year", badge: null,
    borderColor: "rgba(255,255,255,0.12)", glowColor: "rgba(255,255,255,0.06)",
    featured: false, cta: "Apply Now",
    perks: [
      "Private office suite", "Board-room access (unlimited)",
      "Dedicated relationship manager", "Exclusive LP & VC network",
      "Co-branding opportunities", "Priority media placement",
      "IPO readiness consultancy", "Annual summit VIP pass",
    ],
  },
];
...
const FEATURES = [
  {
    ...
    iconPath: <path d="M22 70L62 30 M62 30H45 M62 30V47" stroke="white" strokeWidth="5" strokeLinecap="round" />,
  },
  {
    title: "growth",
    desc: "Structured incubator roadmaps, milestone-driven acceleration tracks, and elite masterclasses.",
    iconPath: <path d="M30 35H62V69H30V35Z M38 35V26C38 19.3726 43.3726 14 50 14C56.6274 14 62 19.3726 62 26V35" stroke="white" strokeWidth="5" />,
  },
  {
    title: "support",
    desc: "Direct 1:1 access to veteran multi-exit mentors alongside full legal, governance, and cap-table help.",
    iconPath: <path d="M20 24h18v44H20z M54 24h18v44H54z M46 18v56" stroke="white" strokeWidth="5" strokeLinecap="round" strokeDasharray="4 4" />,
  },
];
...
function ScatteredCards({ sectionRef, onReady }) {
  ...
  const cardGeometry = useMemo(() => {
    let geo = null;
    cardScene.traverse((c) => { if (c.isMesh && !geo) geo = c.geometry; });
    return geo;
  }, [cardScene]);

  const cardMaterial = useMemo(() => {
    let mat = null;
    cardScene.traverse((c) => { if (c.isMesh && !mat) mat = c.material; });
    return mat;
  }, [cardScene]);
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const st = { trigger: sectionRef.current, start: "top 90%", toggleActions: "play none none none", refreshPriority: 1 };
      const tl = gsap.timeline({ scrollTrigger: st });
      scatterData.forEach((d, i) => {
        tl.to(d, { x: d.targetX, y: d.targetY, z: d.targetZ,
          rotX: d.targetRotX, rotY: d.targetRotY, rotZ: d.targetRotZ,
          duration: 1.5, ease: "expo.out" }, i * 0.02);
      });
      gsap.to(meshRef.current.position, { y: 4, z: 3, ease: "none",
        scrollTrigger: { trigger: sectionRef.current, start: "top bottom",
          end: "bottom top", scrub: 0.5, refreshPriority: 1 } });
    }, sectionRef);
  ...
  useFrame(({ clock }) => {
    ...
    for (let i = 0; i < cardCount; i++) {
      ...
      const rs = rotationStateRef.current[i];
      rs.rotX += d.rotSpeedX;
      rs.rotY += d.rotSpeedY;
...
function MembershipCard({ tier, index, containerRef }) {
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    glowRef.current.style.background =
      `radial-gradient(320px circle at ${e.clientX - left}px ${e.clientY - top}px, ${tier.glowColor}, transparent 70%)`;
  }, [tier.glowColor]);
  ...
  useLayoutEffect(() => {
    ...
    gsap.fromTo(cardRef.current, { y: 60, autoAlpha: 0 }, {
      y: 0, autoAlpha: 1, duration: 1.0, ease: "power3.out", delay: index * 0.15,
      scrollTrigger: { trigger: containerRef.current, start: "top 72%", toggleActions: "play none none reverse" },
    });
  }, [index, containerRef]);

  const btnBase = "w-full py-3.5 text-[0.75rem] font-black tracking-[0.25em] uppercase transition-all duration-300";
  const featuredBtnStyle = { background: "white", color: "black" };
  const ghostBtnStyle   = { background: "transparent", color: "white", border: "1px solid rgba(255,255,255,0.25)" };
  ...
  return (
    ...
    >
      <div ref={glowRef} className="pointer-events-none absolute inset-0 z-0" style={{ transition: "background 0.15s ease" }} />
      ...
      <div className="relative z-10 flex flex-col h-full p-8 lg:p-10">
        <div className="mb-8">
          <p className="text-[0.65rem] tracking-[0.4em] uppercase text-white/35 font-bold mb-2">{tier.tagline}</p>
          <h3 className="font-serif text-[2.2rem] font-medium leading-none tracking-[-0.03em] text-white mb-6 uppercase">{tier.label}</h3>
          <div className="flex items-baseline gap-1">
            <span className="font-serif text-[2.6rem] font-semibold leading-none text-white">{tier.price}</span>
            <span className="text-white/35 text-sm font-light">{tier.period}</span>
          </div>
        </div>
        ...
        <ul className="flex flex-col gap-3.5 flex-1 mb-10">
          {tier.perks.map((perk, i) => (
            <li key={i} className="flex items-start gap-3">
              <span className="mt-[3px] shrink-0 w-[18px] h-[18px] flex items-center justify-center" style={{ border: "1px solid rgba(255,255,255,0.25)" }}>
                {CHECK_ICON}
              </span>
              <span className="text-[0.9rem] leading-snug text-white/60 font-light">{perk}</span>
            </li>
          ))}
        </ul>
        ...
        <button
          className={btnBase}
          style={tier.featured ? featuredBtnStyle : ghostBtnStyle}
          onMouseEnter={(e) => { if (!tier.featured) Object.assign(e.currentTarget.style, { background: "white", color: "black", borderColor: "white" }); }}
          onMouseLeave={(e) => { if (!tier.featured) Object.assign(e.currentTarget.style, ghostBtnStyle); }}
        >
          {tier.cta}
        </button>
      </div>
...
export default function Membershipsection() {
  ...
  const refreshReady = windowLoaded && fontsReady && scatterSceneReady;
  ...
  // window load + fonts ready
  useEffect(() => {
    if (document.readyState === "complete") return;
    const h = () => setWindowLoaded(true);
    window.addEventListener("load", h, { once: true });
  ...
  useEffect(() => { if (refreshReady) return refreshAfterLayout(); }, [refreshReady]);
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.fromTo(headerRef.current,
        ...
        { y: 0, autoAlpha: 1, letterSpacing: "0.42em", duration: 0.7, ease: "power3.out" })
        .fromTo(subheadRef.current, { y: 24, autoAlpha: 0 }, { y: 0, autoAlpha: 1, duration: 0.9, ease: "power3.out" }, "-=0.4")
        .fromTo(statsRef.current,   { y: 20, autoAlpha: 0 }, { y: 0, autoAlpha: 1, duration: 0.8, ease: "power3.out" }, "-=0.5");
    }, sectionRef);
    return () => ctx.revert();
  }, []);
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo(scatterTextRef.current,
        { autoAlpha: 0, y: 40, scale: 0.96 },
        { autoAlpha: 1, y: 0, scale: 1, duration: 0.9, ease: "back.out(1.7)",
          scrollTrigger: { trigger: scatterSectionRef.current, start: "top 75%",
            toggleActions: "play none none reverse", refreshPriority: 0 } });
    }, scatterSectionRef);
    return () => ctx.revert();
  }, []);

  // compare footer reveal
  useFadeIn(compareRef, { start: "top 88%" });
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const tl = gsap.timeline({
        scrollTrigger: { trigger: featureSectionRef.current, start: "top 78%",
          toggleActions: "play none none reverse", refreshPriority: 0 },
      });
      tl.fromTo(featureHeaderRef.current,
        { autoAlpha: 0, y: 18, letterSpacing: "0.55em" },
        { autoAlpha: 1, y: 0, letterSpacing: "0.42em", duration: 0.7, ease: "power3.out" });
      rows.forEach((row, i) => {
        tl.fromTo(row.querySelector(".row-content"), { y: 120, autoAlpha: 0 },
          { y: 0, autoAlpha: 1, duration: 1.1, ease: "power4.out" }, i === 0 ? ">-0.4" : "-=0.9")
          .fromTo(row.querySelector(".row-line"), { scaleX: 0 },
            { scaleX: 1, duration: 1.3, ease: "expo.out" }, "<0.2");
      });
    }, featureSectionRef);
    return () => ctx.revert();
  }, []);
    ...
    <main className="w-full bg-black">
      ...
      <section ref={scatterSectionRef} className="relative h-screen w-full bg-black flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 z-0 h-full w-full">
          <Canvas dpr={[1, 1.25]} gl={{ antialias: false, powerPreference: "high-performance", alpha: false }} frameloop="demand">
        ...
        <div ref={scatterTextRef} className="relative z-10 flex flex-col items-center text-center px-6 max-w-4xl group cursor-default" style={{ opacity: 0 }}>
          ...
          <p className="text-[clamp(1.8rem,4vw,3.5rem)] leading-[1.3] text-white transition-opacity duration-1000 ease-in-out font-light opacity-100 md:opacity-20 md:group-hover:opacity-100">
            ...
            , and enabling over{" "}
            <span className="font-semibold">10,000+ new jobs</span> across competitive tech markets.
          </p>
        </div>
      </section>
      ...
      <section ref={sectionRef} className="relative overflow-hidden bg-black text-white">
        ...
        <div className="relative mx-auto max-w-6xl px-6 py-24 sm:px-10 sm:py-28 lg:px-16 lg:py-32">
          <div className="mb-20 flex items-center gap-6">
            ...
            <span ref={headerRef} className="text-[0.7rem] font-bold tracking-[0.5em] uppercase text-white/40" style={{ opacity: 0 }}>
              Membership Tiers
            </span>
            <span className="h-[1px] flex-1 bg-white/10" />
          </div>

          <div ref={subheadRef} className="text-center mb-6" style={{ opacity: 0 }}>
            <h2 className="font-serif text-[clamp(2.8rem,6vw,5.5rem)] font-semibold leading-[1.0] tracking-[-0.04em] uppercase text-white mx-auto">
              Choose Your<br />
              <span className="italic font-light text-white/60">Growth Path</span>
            </h2>
            <p className="mt-6 mx-auto max-w-[52ch] text-[1rem] leading-relaxed text-white/45 font-light">
              Every membership is a partnership. We grow when you grow — pick the tier that matches your stage, and we'll close the gap to the next.
            </p>
          </div>

          <div ref={statsRef} className="mx-auto max-w-3xl mt-12 mb-20 grid grid-cols-3 divide-x divide-white/10 border border-white/10" style={{ opacity: 0 }}>
            {STATS.map((s, i) => (
              <div key={i} className="flex flex-col items-center py-6 px-4">
                <p className="font-serif text-[clamp(1.6rem,3.5vw,2.4rem)] font-semibold text-white leading-none">
                  <StatCounter target={s.value} suffix={s.suffix} prefix={s.prefix} />
                </p>
                <p className="text-[0.65rem] tracking-[0.2em] uppercase text-white/35 mt-2 font-bold">{s.label}</p>
              </div>
            ))}
          </div>
          ...
          <div ref={cardsContainerRef} className="grid gap-5 md:grid-cols-3">
            {TIERS.map((tier, i) => (
              <MembershipCard key={tier.id} tier={tier} index={i} containerRef={cardsContainerRef} />
            ))}
          </div>

          <div ref={compareRef} className="mt-14 flex flex-col sm:flex-row items-center justify-center gap-4 text-center" style={{ opacity: 0 }}>
            <p className="text-white/30 text-sm font-light">All memberships include GST. Need a custom enterprise plan?</p>
      ...
      <section ref={featureSectionRef} className="relative overflow-hidden bg-black text-white">
        ...
        <div className="relative mx-auto max-w-6xl px-6 py-24 sm:px-10 sm:py-28 lg:px-16 lg:py-32">
          <div className="mb-20 flex items-center gap-6">
            <span className="h-[1px] flex-1 bg-white/10" />
            <span ref={featureHeaderRef} className="text-[0.7rem] font-bold tracking-[0.5em] uppercase text-white/40">
              Why Startup Park
            </span>
            <span className="h-[1px] flex-1 bg-white/10" />
          </div>
          ...
          <div className="flex flex-col">
            {FEATURES.map((item, i) => (
              <div key={i} ref={(n) => { featureRowsRef.current[i] = n; }} className="relative overflow-hidden group">
